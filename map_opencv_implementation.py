...
def rgb2yuv0(img : np.ndarray):
    if img.dtype == np.uint8:
        return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
        return np.sum(img * np.array([[[0.299, 0.587, 0.114]]]), axis=2)
...
for frame_number in range(1, num_images):
    ...
    current_frame_gray = rgb2yuv0(current_frame)
    prev_frame_gray = rgb2yuv0(prev_frame)
    flow = cv2.DISOpticalFlow_create(2)
    flow.setFinestScale(0)
    # Note: must be uint8 dtype
    current_flow = flow.calc(prev_frame_gray.clip(0, 255).astype(np.uint8),
                             current_frame_gray.clip(0, 255).astype(np.uint8), None)
    ...
    aligned_frame_gray = rgb2yuv0(aligned_frame)
...
video.open(os.path.join(output_folder, "out.mp4"), video.fourcc('m', 'p', '4', 'v'), 30, (w, h), 1)
for i in range(1, 100):
    filename = f'{i:08d}.png'  # Format the filename (e.g., 0000000.png)
